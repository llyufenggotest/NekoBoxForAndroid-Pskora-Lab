...
fun parseV2Ray(link: String): StandardV2RayBean {
    if (link.startsWith("xless://", ignoreCase = true)) return parsePureLink(link)
...
fun StandardV2RayBean.toUriVMessVLESSTrojan(isTrojan: Boolean): String {
    ...
    if (name.isNotBlank()) {
        if (this is VMessBean && isPure()) builder.fragment(name)
        else builder.encodedFragment(name.urlSafe())
    }
...
fun buildSingBoxOutboundStreamSettings(bean: StandardV2RayBean): V2RayTransportOptions? {
    if (bean is VMessBean && bean.isPure()) {
        bean.validatePureOptions()
