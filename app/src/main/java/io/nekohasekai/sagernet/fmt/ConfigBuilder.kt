...
import moe.matsuri.nb4a.proxy.config.nativeUrlTest
...
class ConfigBuildResult(
    ...
    val preferredRuntimeTags: MutableMap<Long, String> = mutableMapOf(),
    val preferredRuntimeMembers: MutableMap<String, Map<String, Long>> = mutableMapOf(),
) {
...
fun buildConfig(
    ...
    if (proxy.type == TYPE_CONFIG) {
        ...
        if (bean.type == 0) {
            ...
            return ConfigBuildResult(
                if (forTest) disableTestCache(bean.config) else bean.config,
                listOf(),
                proxy.id, //
    ...
    val preferredTags = mutableMapOf<Long, String>()
    val preferredMembers = mutableMapOf<String, Map<String, Long>>()
    ...
    return MyOptions().apply {
        experimental = buildExperimentalOptions(forTest, DataStore.enableClashAPI)
        ...
        fun buildChain(
            ...
            require(profileList.none { it.configBean?.type == 2 }) { "链式/前置/落地代理不能嵌入优选分组，请把链式节点作为优选候选" }
            ...
            profileList.forEachIndexed { index, proxyEntity ->
                ...
                // Resolve a shared terminal hop before linking its predecessor.
                // A cached single node may have a readable tag instead of g-<id>.
                tagOut = resolveSharedOutboundTag(tagOut, needGlobal, proxyEntity.id, globalOutbounds)
        ...
        // Membership is refreshed on every config build, never expanded into persisted ids.
        val preferredResolver by lazy {
            PreferredGroupResolver(SagerDatabase.proxyDao.getAll(), SagerDatabase.groupDao.allGroups())
        }
        fun buildProfile(entity: ProxyEntity, chainId: Long): String {
            val bean = entity.requireBean()
            if (bean !is ConfigBean || bean.type != 2) return buildChain(chainId, entity)
            preferredTags[entity.id]?.let { return it }
            preferredResolver.validate(entity)
            val members = preferredResolver.members(entity)
            val tags = members.map { buildProfile(it, it.id) }
            val tag = readableTag(entity.displayName())
            val spec = bean.preferredSpec()
            outbounds.add(CustomSingBoxOption(gson.toJson(nativeUrlTest(tag, tags,
                spec.intervalSeconds, spec.minDelayMilliseconds, spec.mode))) as SingBoxOption)
            trafficMap[tag] = listOf(entity) // Leaf tags already own their traffic; never count it twice.
            preferredMembers[tag] = tags.zip(members.map { it.id }).toMap()
            preferredTags[entity.id] = tag
            return tag
        }
        ...
        if (buildSelector) {
            ...
            list.forEach {
                tagMap[it.id] = buildProfile(it, it.id)
            }
            ...
            val mainTag = buildProfile(proxy, 0)
            tagMap[proxy.id] = mainTag
        }
        // build outbounds from route item
        extraProxies.forEach { (key, p) ->
            tagMap[key] = buildProfile(p, key)
        }

        val mainProxyTag = (if (buildSelector) TAG_PROXY else tagMap[proxy.id]) ?: TAG_PROXY
        // Preferred candidates precede their urltest in outbounds; default must target
        // the selected profile explicitly, never the first candidate.
