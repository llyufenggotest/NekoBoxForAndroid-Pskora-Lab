...
object GroupManager {
    ...
    suspend fun deleteGroup(groupId: Long) {
        val selectedBelongsToGroup = SagerDatabase.proxyDao.getById(DataStore.selectedProxy)?.groupId == groupId
        SagerDatabase.instance.runInTransaction {
            SagerDatabase.groupDao.deleteById(groupId)
            SagerDatabase.proxyDao.deleteByGroup(groupId)
        }
        if (selectedBelongsToGroup) DataStore.selectedProxy = 0L
    ...
    suspend fun deleteGroup(group: List<ProxyGroup>) {
        ...
        val selectedBelongsToGroup = SagerDatabase.proxyDao.getById(DataStore.selectedProxy)?.groupId in groupIds.toSet()
        SagerDatabase.instance.runInTransaction {
