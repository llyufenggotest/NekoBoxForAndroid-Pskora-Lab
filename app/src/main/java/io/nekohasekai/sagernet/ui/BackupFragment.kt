...
class BackupFragment : NamedFragment(R.layout.layout_backup) {
    ...
    fun finishImport(
        ...
        if (profile && content.has("profiles")) {
            ...
            // Full replacement preserves original non-zero IDs (same contract as ChainBean).
            // Validate against the incoming snapshot BEFORE deleting any existing data.
            moe.matsuri.nb4a.proxy.config.validatePreferredRestoreSnapshot(profiles, groups)
