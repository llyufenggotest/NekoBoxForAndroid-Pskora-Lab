        ...
        while (currentCoroutineContext().isActive) {
            ...
            val snapshot = withStateLock {
                ...
                tagMap.values.forEach {
                    // The native tracker counts the routed outbound, including urltest.
                    // Its internal candidate is not a second tracked connection.
                    if (!it.ignore) {
                        mainTxRate += it.txRate
                        mainRxRate += it.rxRate
                    }
                    // Keep completed traffic when selector switches away from a profile.
