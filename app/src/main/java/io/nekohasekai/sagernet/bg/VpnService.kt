...
//        val tunOptions = JSONObject(tunOptionsJson)
    ...
    /** A declares initial/live, S initial only, I omits both framework call sites. */
    fun updateUnderlyingNetwork(builder: Builder? = null) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP_MR1) return
        val network = SagerNet.underlyingNetwork
        if (builder != null) {
            if (io.nekohasekai.sagernet.BuildConfig.VPN_UNDERLYING_INITIAL) {
                network?.let { builder.setUnderlyingNetworks(arrayOf(it)) }
            } else {
                VpnUnderlyingLiveDiagnostic.recordInitialOmitted(network)
            }
        } else if (io.nekohasekai.sagernet.BuildConfig.VPN_UNDERLYING_LIVE) {
            network?.let {
                VpnUnderlyingLiveDiagnostic.recordLiveSet(it) { networks ->
                    setUnderlyingNetworks(networks)
                }
            }
        } else {
            VpnUnderlyingLiveDiagnostic.recordSuppressedEstablishedUpdate(network)
        }
    }
