...
object SendLog {
    ...
    fun getNekoLog(max: Long): ByteArray = readNekoLog(
        File(SagerNet.application.cacheDir, "neko.log"),
        max,
    )

    /** Package-visible contract boundary used by export sink tests. */
    internal fun readNekoLog(file: File, max: Long): ByteArray {
        return try {
            val len = file.length()
