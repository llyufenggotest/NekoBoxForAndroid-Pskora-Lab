...
type Client struct {
	...
	isX365 bool // X365 private framing (#x365)
	isJuzi bool // Juzi SDK tag framing (#juzi, case-insensitive)
	isPure bool // Pure A1 framing, TCP only; never standard UDP/XUDP.
}

func splitPrivateSuffix(userID string) (cleanID string, isX365 bool, isJuzi bool) {
	lowerID := strings.ToLower(userID)
	switch {
	case strings.HasSuffix(lowerID, "#juzi"):
		return userID[:len(userID)-len("#juzi")], false, true
	case strings.HasSuffix(userID, "#x365"):
		return userID[:len(userID)-len("#x365")], true, false
	default:
...
func NewClient(userId string, flow string, logger logger.Logger) (*Client, error) {
	key, pure, pureErr := PureIdentity(userId)
	if pureErr != nil {
		return nil, pureErr
	}
	if pure {
		if flow != "" {
			return nil, E.New("Pure: flow/Vision unsupported")
		}
		return &Client{key: key, logger: logger, isPure: true}, nil
	}
	userId, isX365, isJuzi := splitPrivateSuffix(userId)
	...
	return &Client{key: user, flow: flow, logger: logger, isX365: isX365, isJuzi: isJuzi}, nil
}
...
func (c *Client) DialConn(conn net.Conn, destination M.Socksaddr) (net.Conn, error) {
	if c.isPure {
		return c.dialPure(conn, destination)
	}
	remoteConn := NewConn(conn, c.key, vmess.CommandTCP, destination, c.flow, c.isX365, c.isJuzi)
...
func (c *Client) DialEarlyConn(conn net.Conn, destination M.Socksaddr) (net.Conn, error) {
	...
	return c.prepareConn(NewConn(conn, c.key, vmess.CommandTCP, destination, c.flow, c.isX365, c.isJuzi), conn)
}
...
func (c *Client) DialEarlyConnWithOptions(conn net.Conn, baseConn net.Conn, destination M.Socksaddr, canSplice bool) (net.Conn, error) {
	...
	return c.prepareConnWithOptions(NewConn(conn, c.key, vmess.CommandTCP, destination, c.flow, c.isX365, c.isJuzi), baseConn, canSplice)
}

func (c *Client) DialPacketConn(conn net.Conn, destination M.Socksaddr) (*PacketConn, error) {
	if c.isPure {
		return nil, E.New("Pure: UDP/XUDP unsupported")
	}
	serverConn := &PacketConn{Conn: conn, key: c.key, destination: destination, flow: c.flow, isX365: c.isX365, isJuzi: c.isJuzi}
...
func (c *Client) DialEarlyPacketConn(conn net.Conn, destination M.Socksaddr) (*PacketConn, error) {
	...
	return &PacketConn{Conn: conn, key: c.key, destination: destination, flow: c.flow, isX365: c.isX365, isJuzi: c.isJuzi}, nil
}
...
func (c *Client) DialEarlyXUDPPacketConn(conn net.Conn, destination M.Socksaddr) (vmess.PacketConn, error) {
	...
	remoteConn := NewConn(conn, c.key, vmess.CommandMux, destination, c.flow, c.isX365, c.isJuzi)
	protocolConn, err := c.prepareConn(remoteConn, conn)
	if err != nil {
		return nil, err
	}
...
func NewConn(conn net.Conn, uuid [16]byte, command byte, destination M.Socksaddr, flow string, isX365 bool, isJuzi bool) *Conn {
	return &Conn{
...
func (c *Conn) Read(b []byte) (n int, err error) {
	if !c.responseRead {
		err = ReadResponse(c.ExtendedConn, c.request.IsX365)
		if err != nil {
			return
		}
		c.responseRead = true
...
func (c *PacketConn) Read(b []byte) (n int, err error) {
	if !c.responseRead {
		err = ReadResponse(c.Conn, c.isX365)
		if err != nil {
			return
		}
		c.responseRead = true
...
func (c *PacketConn) Write(b []byte) (n int, err error) {
	if !c.requestWritten {
		...
		if c.requestWritten {
			...
			err = WritePacketRequest(c.Conn, Request{c.key, vmess.CommandUDP, c.destination, c.flow, c.isX365, c.isJuzi}, nil)
			if err == nil {
				n = len(b)
			}
			c.requestWritten = true
...
func (c *PacketConn) WritePacket(buffer *buf.Buffer, destination M.Socksaddr) error {
	...
	if !c.requestWritten {
		...
		if c.requestWritten {
			...
			err := WritePacketRequest(c.Conn, Request{c.key, vmess.CommandUDP, c.destination, c.flow, c.isX365, c.isJuzi}, buffer.Bytes())
			c.requestWritten = true
			c.access.Unlock()
			return err
		}
