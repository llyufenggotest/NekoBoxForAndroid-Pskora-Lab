...
type androidSelectionListener struct{}

func (androidSelectionListener) OnSelectorChanged(selector, tag string) {
	if intfNB4A != nil {
		intfNB4A.Selector_OnProxySelected(selector, tag)
	}
}
...
func ResetAllConnections(system bool) {
	if system {
		if mainInstance != nil {
			mainInstance.connection.CloseAll()
		}
...
func NewSingBoxInstance(config string, localTransport LocalDNSTransport) (b *BoxInstance, err error) {
	...
	service.MustRegister[group.SelectionListener](ctx, androidSelectionListener{})

	// parse options
	var options option.Options
	ctx, adapted, adaptErr := configcompat.ConvertContext(ctx, []byte(config), nekoutils.GetGeoIPHeadlessRules, nekoutils.GetGeoSiteHeadlessRules)
	if adaptErr != nil {
		cancel()
		return nil, fmt.Errorf("adapt config: %w", adaptErr)
	}
	err = options.UnmarshalJSONContext(ctx, adapted)
	if err != nil {
