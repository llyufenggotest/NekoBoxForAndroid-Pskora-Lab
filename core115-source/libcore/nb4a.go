...
// NekoLogReconfigure changes only logging, without reinitializing core globals.
// Callers quiesce old core/helper producers before clearing; both app processes
// must update their own gate. Errors propagate through gomobile as exceptions.
func NekoLogReconfigure(enabled, clear bool) error {
	return neko_log.Reconfigure(enabled, clear)
}
