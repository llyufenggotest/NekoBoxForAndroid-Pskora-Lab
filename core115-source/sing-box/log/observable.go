...
// DiagnosticEnabled is deliberately independent of formatting and only permits debug/trace.
func (f *defaultFactory) DiagnosticEnabled() bool {
	return f.diagnosticEnabled.Load()
}
...
func (f *defaultFactory) SetLevel(level Level) {
	// The current-generation flow diagnostic is metadata-only, bounded, and
	// available only when the user explicitly selects Debug/Trace. It does not
	// change routing, recovery, retry, URLTest, or transport behavior.
	f.diagnosticEnabled.Store(level >= LevelDebug)
	f.level = level
}

func (f *defaultFactory) RecoveryDiagnosticEnabled() bool {
