...
func (r *Router) routeConnection(ctx context.Context, conn net.Conn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) error {
	...
	if metadata.InboundType == C.TypeTun {
		...
		if metadata.ProcessInfo != nil {
			packages = metadata.ProcessInfo.PackageNames
			uid = metadata.ProcessInfo.UserId
			ownerKnown = true
		}
		ctx = D.BeginTUN(ctx, r.logger, metadata.Source.Port, packages, uid, ownerKnown)
		if generationManager, loaded := r.connection.(contextualTUNTCPGenerationManager); loaded {
			conn = generationManager.TrackTUNTCPContext(ctx, conn)
		} else if generationManager, loaded := r.connection.(TUNTCPGenerationManager); loaded {
			conn = generationManager.TrackTUNTCP(conn)
		}
	}
	...
	D.EventFrom(ctx, r.logger, D.Record{Event: D.Route})
