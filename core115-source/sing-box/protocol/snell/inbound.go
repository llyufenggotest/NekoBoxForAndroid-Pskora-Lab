...
func (h *Inbound) NewConnection(ctx context.Context, conn net.Conn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) {
	if h.obfsMode != "" {
		mode, _ := snellprotocol.ParseObfsMode(h.obfsMode)
		conn = (snellprotocol.ObfsConfig{Mode: mode}).ServerConn(conn)
	}
...
func validateSnellInboundObfs(version int, obfsMode string) error {
	if version != 5 || obfsMode == "" || obfsMode == "none" || obfsMode == "http" || obfsMode == "tls" {
		return nil
	}
	if obfsMode == "tls" {
