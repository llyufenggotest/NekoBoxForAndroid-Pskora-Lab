...
func (r *Router) lookupWithRules(ctx context.Context, rules []adapter.DNSRule, domain string, options adapter.DNSQueryOptions) ([]netip.Addr, error) {
	...
	if strategy == C.DomainStrategyPreferIPv4 || strategy == C.DomainStrategyPreferIPv6 {
		return lookupPreferredFamilies(ctx, strategy, func(ctx context.Context, q uint16) ([]netip.Addr, error) {
			return r.lookupWithRulesType(ctx, rules, domain, q, lookupOptions)
		})
	}
	var (
...
func defaultRuleDisablesLegacyDNSMode(rule option.DefaultDNSRule) bool {
	return rule.MatchResponse.IsEnabled() ||
		...
		rule.IPVersion > 0 ||
		len(rule.QueryType) > 0 && !rule.LegacyAndroidQueryType
}
