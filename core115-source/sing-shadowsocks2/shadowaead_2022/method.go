...
import (
	...
	"encoding/base64"
	"encoding/binary"
	// "encoding/hex" // 【已删除】避免编译报错 imported and not used
	"math"
	mRand "math/rand"
	"net"
	"os"
...
// 【新增】黑石模式全局开关
var EnableBlackstoneHack = false 
...
func NewMethod(ctx context.Context, methodName string, options C.MethodOptions) (C.Method, error) {
	m := &Method{
		...
		rawPassword: options.Password, // 记录原始密码，供后续判断
	}

	// 1. 先判断开关 (只要包含这个词就开启，不管是怎么拼接的)
	if strings.Contains(options.Password, "BLACKSTONE") {
		EnableBlackstoneHack = true
	}

	if options.Password != "" {
		// 【优化点 1】 先把干扰词 BLACKSTONE 彻底删掉
		// 这样无论它是 #BLACKSTONE 还是直接粘在后面的 BLACKSTONE，都会被清理
		cleanPass := strings.ReplaceAll(options.Password, "BLACKSTONE", "")
		
		var pskList [][]byte
		keyStrList := strings.Split(cleanPass, ":")
		pskList = make([][]byte, 0, len(keyStrList))

		for _, keyStr := range keyStrList {
			// 【优化点 2】 双重保险：还是保留处理 # 的逻辑，以防万一有其他 Tag
			cleanKeyStr := keyStr
			if idx := strings.Index(cleanKeyStr, "#"); idx != -1 {
				cleanKeyStr = cleanKeyStr[:idx]
			}
			
			// 【优化点 3】 去除可能存在的首尾空格（防止配置复制带入空格导致解码失败）
			cleanKeyStr = strings.TrimSpace(cleanKeyStr)
			
			// 跳过空项（防止 "key::key" 这种中间空的情况）
			if len(cleanKeyStr) == 0 {
				continue
			}

			kb, err := base64.StdEncoding.DecodeString(cleanKeyStr)
			if err != nil {
				// 这里加个日志打印，方便以后排查到底是哪段字串出了问题
				// fmt.Printf("Decode Error String: %s\n", cleanKeyStr) 
...
func (c *clientConn) writeRequest(payload []byte) error {
	...
	// 3. 准备 MagicCode
	isBlackstone := strings.Contains(c.method.rawPassword, "BLACKSTONE")
	magicCode := []byte{}
	if isBlackstone {
		targetKey := c.method.pskList[len(c.method.pskList)-1]
		b64Key := base64.StdEncoding.EncodeToString(targetKey)
		hash := md5.Sum([]byte(b64Key))
		magicCode = hash[:] 
	}

	// 4. 构造 Fixed Header
	// 标准 11，黑石模式加 16，总共 27 (完美契合 Frida 证据)
	headerLen := RequestHeaderFixedChunkLength
	if len(magicCode) > 0 {
		headerLen += len(magicCode)
	}

	// 申请 headerLen 长度的 buffer
	fixedLengthBuffer := buf.With(requestBuffer.Extend(headerLen + shadowio.Overhead))
	...
	// B. 【关键】写入 MagicCode (后16字节) 
	// 紧跟在标准 Header 后面
	if len(magicCode) > 0 {
		common.Must1(fixedLengthBuffer.Write(magicCode))
	}
	
	// 5. 加密 Fixed Header
	// 加密范围：索引 0 开始，包含标准头和 MagicCode
	writer.Encrypt(fixedLengthBuffer.Index(0), fixedLengthBuffer.Bytes())
	
	fixedLengthBuffer.Extend(shadowio.Overhead)

	// 6. 处理 Variable Header (保持原样)
