...
fun Project.setupApp() {
    ...
    android.apply {
        defaultConfig {
            ...
            // Compile-time A/S/I experiment. I is the safe default:
            // A declares initially and live, S declares initially only, I never declares.
            val vpnUnderlyingVariant = project.findProperty("vpnUnderlyingVariant")?.toString()?.uppercase() ?: "I"
            check(vpnUnderlyingVariant in setOf("A", "S", "I"))
            buildConfigField("boolean", "VPN_UNDERLYING_INITIAL", (vpnUnderlyingVariant != "I").toString())
            buildConfigField("boolean", "VPN_UNDERLYING_LIVE", (vpnUnderlyingVariant == "A").toString())
            buildConfigField("String", "VPN_UNDERLYING_VARIANT", "\"$vpnUnderlyingVariant\"")
        }
    }
    setupAppCommon()
            ...
            outputs.all {
                ...
                outputFileName = if (isPreview) {
                    outputFileName.replace(
                        project.name,
                        "NekoBoxYF-" + requireMetadata().getProperty("PRE_VERSION_NAME")
                    ).replace("-preview", "")
                } else {
                    outputFileName.replace(project.name, "NekoBoxYF-$versionName")
                        .replace("-release", "")
                        .replace("-oss", "")
                }
            }
